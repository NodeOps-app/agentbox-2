...
agentbox manager start --agent codex --new
agentbox manager status
agentbox manager attach                     # or --attach-in split|window|tab
agentbox manager sessions
agentbox manager stop
```

The manager runs in a tmux session the hub owns, so the CLI, the tray and a plain
`tmux attach` all reach the same running agent. It needs `tmux` on the machine running the hub;
`agentbox doctor` tells you if it is missing.
