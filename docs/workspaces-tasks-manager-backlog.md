...
| # | Item | Size | Notes |
|---|------|------|-------|
| 11 | **+/− lines on push rows** | done | `git.push` events carry `additions`/`deletions` (`pushLineStat`, `packages/relay/src/workspaces/push-stat.ts`): the pushed ref's tip is read in the host repo BEFORE the push (`refs/remotes/<remote>/<branch>`, or `refs/heads/<dest>` for push-host), then `git diff --shortstat old..new`; with no old tip, from the merge base with `origin/HEAD` → `origin/main\|master` → `main\|master`. One 2 s budget, undefined on any failure or an empty diff. Wired in the docker `git.push` RPC (`server.ts`), the cloud executor (`runGitRpc`; skipped for a scratch repo) and the hub's `push` / `push-host` routes (`recordAround`). OpenAPI + api.mdx updated. **Tray:** the event's `additions`/`deletions` are top-level; `TimelineItem` folds them only into a `pr`, so a push row needs to keep them. |
